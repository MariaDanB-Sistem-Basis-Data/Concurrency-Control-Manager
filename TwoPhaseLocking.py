...
class TwoPhaseLocking(ConcurrencyMethod):
    ...
    def log_object(self, object: Row, transaction_id: int) -> None:
        ...
        transaction = self.transaction_manager.getTransaction(transaction_id)
        if not transaction:
    ...
    def validate_object(self, object: Row, transaction_id: int, action: Action) -> Response:
        ...
        op = Operation(transaction_id=transaction_id, resource_id=resource_id, operation_type=action)
    ...
    def end_transaction(self, transaction_id: int) -> None:
        ...
        transaction = self.transaction_manager.get(transaction_id)
        if not transaction:
        ...
        try:
            self.transaction_manager.pop(transaction_id, None)
        except Exception as e:
